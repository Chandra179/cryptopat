...
class ChaikinMoneyFlow:
    ...
    def calculate(self):
        ...
        if len(self.ohlcv) < self.rules["cmf_period"]:
            result["error"] = f"Insufficient data: need at least {self.rules['cmf_period']} candles, got {len(self.ohlcv)}"
        ...
        for candle in self.ohlcv:
            ...
            mf_multiplier = self.rules["money_flow_multiplier_formula"](high, low, close)
        ...
        for i in range(period - 1, len(self.ohlcv)):
            ...
            # Calculate CMF
            cmf = self.rules["cmf_formula"](mf_volume_sum, volume_sum)
            cmf_values.append({
        ...
        if cmf_values:
            ...
            if latest_cmf > self.rules["strong_buying_threshold"]:
                result["interpretation"] = f"Strong buying pressure (CMF: {latest_cmf:.4f})"
            elif latest_cmf < self.rules["strong_selling_threshold"]:
    ...
    def print_output(self, result: dict):
        ...
        print(f"CMF Period: {self.rules['cmf_period']}")
        ...
        print(f"  > {self.rules['strong_buying_threshold']}: Strong buying pressure")
        print(f"  0 to {self.rules['strong_buying_threshold']}: Moderate buying pressure")
        print(f"  {self.rules['strong_selling_threshold']} to 0: Moderate selling pressure")
        print(f"  < {self.rules['strong_selling_threshold']}: Strong selling pressure")
...
class SharkPatternDetector:
    ...
    def detect_pattern(self, symbol: str, timeframe: str = '4h', limit: int = 150) -> Dict:
        """
        Detect Shark pattern for given symbol and timeframe.
        ...
        try:
            ...
            logger.error(f"Error detecting Shark pattern for {symbol}: {e}")
            return {"error": str(e)}
...
def analyze_shark_pattern(symbol: str, timeframe: str = '4h', limit: int = 150, 
                         zigzag_threshold: float = 5.0) -> str:
    """
    Analyze Shark pattern for a symbol.
    ...
    Returns:
        Formatted analysis string
    """
    detector = SharkPatternDetector(zigzag_threshold)
...
if __name__ == "__main__":
    # Example usage
    result = analyze_shark_pattern("XRP/USDT", "4h", 150, 5.0)
    print(result)
...
def run_vwap_analysis(symbol: str = "BTC/USDT", timeframe: str = "1h", limit: int = 100, anchor: Optional[str] = None) -> str:
    """
    ...
    try:
        ...
        # Format output
        result = format_vwap_output(signals)
        
        # Add header with analysis info
        anchor_info = f" (Anchored from {anchor})" if anchor else ""
        header = f"VWAP Analysis for {symbol} {timeframe} (Last {limit} candles){anchor_info}\n" + "="*80 + "\n"
        
        return header + result
        
    except Exception as e:
        return f"Error in VWAP analysis: {str(e)}"


if __name__ == "__main__":
    # Test the VWAP analysis
    result = run_vwap_analysis("ETH/USDT", "15m", 50)
    print(result)